...
class NearClass {
  ...
  public init(callback?: () => void) {
    const config = getConfig(process.env.NEXT_PUBLIC_APP_ENV || "development");
    const near = new Near({
      keyStore: new keyStores.BrowserLocalStorageKeyStore(),
      ...config,
    });
    const wallet = new WalletConnection(near, "dnft");
  ...
  public async executeMultipleTransactions(
    ...
    return this.wallet.requestSignTransactions({transactions:nearTransactions, callbackUrl});
  }
}
